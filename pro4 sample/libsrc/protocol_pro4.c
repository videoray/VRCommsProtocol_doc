...
uint8_t protocol_pro4_checksum(uint8_t* buf, unsigned int len) {
    uint8_t xsum=0;
    unsigned int i=0;
    for (i=0;i<len;i++) {
        xsum ^= buf[i];
...
unsigned int protocol_pro4_parse(uint8_t data,
    ...
    switch(parse_state) {
          ...
          case UNSYNC:
            if ((data==(uint8_t) SYNC_REQUEST_1) || (data== (uint8_t) SYNC_RESPONSE_1)) {
                /* got 1st sync byte */
                index=0;
                checksum=data;
          ...
          case PRO4_SYNC:
            if ((data==(uint8_t) SYNC_REQUEST_2) || (data== (uint8_t) SYNC_RESPONSE_2)) {
                /* got 2nd sync byte */
...
void 
protocol_pro4_handler(Protocol_PRO4_Packet_Header *header, uint8_t* data) {
